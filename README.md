...
| `SUNRAY_ALIAS_STRATEGY` | `slot` | Transient memory aliasing algorithm. `slot` reuses a bucket once its occupant's lifetime ends, every member at offset 0; `bucket` packs lifetime-disjoint resources at offsets inside one bucket, roughly halving transient VRAM at a higher compile cost; `off` disables aliasing entirely (one allocation per resource) — a memory-hungry bisect knob for ruling aliasing in or out of a corruption. Affects memory only, never what is rendered |
