...
This project was developed by [Riccardo-Finello](https://github.com/riccardoFinelloUniTn) and [Matteo-Ballardin](https://github.com/MatteoBallardin)  supervised by Professor [Marco Patrignani](https://squera.github.io/) for the bachelor thesis at the University of Trento, Italy
<br>
...
| `SUNRAY_ALIAS_STRATEGY` | `bucket` | Transient memory aliasing algorithm. `bucket` packs lifetime-disjoint resources at offsets inside one bucket, roughly halving transient VRAM at a higher compile cost; `slot` reuses a bucket once its occupant's lifetime ends, every member at offset 0; `off` disables aliasing entirely (one allocation per resource) — a memory-hungry bisect knob for ruling aliasing in or out of a corruption. Affects memory only, never what is rendered |
| `SUNRAY_STRIP` | off (full pipeline) | Diagnostic bisect: build only the first N stages of the unified graph, leaving acquire → blit → present intact. `0` = no passes at all, `1` = + staging copies and AS builds, `2` = + RIS, `3` = + final shading, `4` = + temporal accumulation, `5` = + denoise. Anything below `6` renders garbage — the blit source is never written, so rungs below `6` also trip a barrier VUID. **Temporary:** exists only to bisect the driver crash in `docs/NVIDIA_BUG_REPORT.md` and is to be deleted once that is fixed |
| `SUNRAY_PRESENT_MODE` | lowest-latency supported | Swapchain present mode: `fifo`, `fifo_relaxed`, `mailbox`, `immediate`. Overrides both the auto-pick and any caller preference. Diagnostic knob — the driver crash in `docs/NVIDIA_DRIVER_CRASH_REPORT.md` lives on the present worker thread |
...
Read once at `Renderer` construction, except `SUNRAY_GRAPH_DUMP_DIR`,
`SUNRAY_ALIAS_STRATEGY` and `SUNRAY_STRIP` (per frame) and `SUNRAY_SHADER_DEBUG`
(build script).
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts/gpu-check.ps1
```

It mirrors the `gpu` job step for step and needs a real RT-capable GPU. If you change a
step in one, change it in the other.
...
| [sunray](https://github.com/Kalsifer-742/sunray)                     |       ✅       |     ✅      |    ✅     |        ✅        |   ✅   | ✅  |  ✅   |   ✅    |  ❌  | ✅  |  ✅  |  partial  |   ✅    |  ✅  |  ✅   |  ✅ Bevy   |     Bevy integration is experimental |
